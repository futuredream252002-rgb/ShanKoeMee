...
const STORAGE_THEME = "trx_theme_mode";
const STORAGE_LOGIN = "trx_last_login";
...
function getPlotBounds() {
    ...
    const volBottom = chartHeight - bottomPad;
    return { plotW, plotTop, plotBottom, volBottom };
}
...
function getPointerXY(evt) {
    ...
    let mouseX, mouseY;
    if (isVerticalView) {
        mouseX = clientY - rect.top;
        mouseY = chartHeight - (clientX - rect.left);
    } else {
...
function formatPeriodLabel(period) {
    ...
    if (s.length >= 4) {
        ...
        return last4.replace(/(\d{2})(\d{2})/, '$1:$2');
    }
    return s || '-';
}
...
function drawChart() {
    ...
    const palette = isLightTheme ? {
        ...
        bgBottom: '#eef2f7',
        emptyText: '#64748b',
        axisBg: 'rgba(15,23,42,0.04)',
        axisLine: 'rgba(15,23,42,0.10)',
        gridMain: 'rgba(15,23,42,0.10)',
        gridSub: 'rgba(15,23,42,0.07)',
        axisLabelBg: 'rgba(255,255,255,0.96)',
        axisLabelStroke: 'rgba(15,23,42,0.10)',
        axisLabelText: '#334155',
        candleNumber: 'rgba(15,23,42,0.92)',
        lastLine: 'rgba(15,23,42,0.25)',
        lastBoxBg: 'rgba(255,255,255,0.94)',
        lastBoxStroke: 'rgba(15,23,42,0.14)',
        lastBoxText: '#0f172a',
        crosshair: 'rgba(15,23,42,0.22)',
        priceBoxBg: 'rgba(59,130,246,0.16)',
        priceBoxStroke: 'rgba(59,130,246,0.30)',
        priceBoxText: '#1d4ed8',
        timeBoxBg: 'rgba(255,255,255,0.96)',
        timeBoxStroke: 'rgba(15,23,42,0.12)',
        timeBoxText: '#334155'
    } : {
        bgTop: '#07080b',
        bgBottom: '#0b0d12',
        emptyText: '#666',
        axisBg: 'rgba(255,255,255,0.03)',
        axisLine: 'rgba(255,255,255,0.06)',
        gridMain: 'rgba(255,255,255,0.06)',
        gridSub: 'rgba(255,255,255,0.045)',
        axisLabelBg: 'rgba(0,0,0,0.55)',
        axisLabelStroke: 'rgba(255,255,255,0.08)',
        axisLabelText: '#cfd8dc',
        candleNumber: 'rgba(255,255,255,0.92)',
        lastLine: 'rgba(255,255,255,0.22)',
        lastBoxBg: 'rgba(255,255,255,0.12)',
        lastBoxStroke: 'rgba(255,255,255,0.16)',
        lastBoxText: '#ffffff',
        crosshair: 'rgba(255,255,255,0.18)',
        priceBoxBg: 'rgba(21,101,192,0.35)',
        priceBoxStroke: 'rgba(79,195,247,0.35)',
        priceBoxText: '#e3f2fd',
        timeBoxBg: 'rgba(0,0,0,0.55)',
        timeBoxStroke: 'rgba(255,255,255,0.10)',
        timeBoxText: '#cfd8dc'
    };
    ...
    ctx.fillRect(plotW, 0, PRICE_AXIS_W, chartHeight);
    ctx.strokeStyle = palette.axisLine;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(plotW + 0.5, 0);
    ctx.lineTo(plotW + 0.5, chartHeight);
    ctx.stroke();

    const candleW = getCandleWidth();
    const gap = Math.max(1, candleW * 0.12);
    const totalStep = candleW + gap;
    const viewLeft = 0;
    const viewRight = plotW;

    let firstVisible = 0;
    let lastVisible = candleData.length - 1;
    for (let i = 0; i < candleData.length; i++) {
        const x = offsetX + (i * totalStep) + (candleW / 2);
        if (x + candleW / 2 >= viewLeft) { firstVisible = i; break; }
    }
    for (let i = candleData.length - 1; i >= 0; i--) {
        const x = offsetX + (i * totalStep) + (candleW / 2);
        if (x - candleW / 2 <= viewRight) { lastVisible = i; break; }
    }
    if (lastVisible < firstVisible) { firstVisible = 0;
        lastVisible = candleData.length - 1; }
    ...
    ctx.strokeStyle = palette.gridMain;
    ctx.lineWidth = 1;
    const hLines = 6;
    ...
    const approxCols = Math.max(6, Math.floor(plotW / 120));
    const colStepPx = plotW / approxCols;
    ctx.strokeStyle = palette.gridSub;
    ...
    for (let i = 0; i <= hLines; i++) {
        const y = plotTop + (i * plotH / hLines);
        const price = maxPrice - (i * priceRange / hLines);
        const txt = price.toFixed(3);
        const tx = plotW + 10;
        const ty = y - 9;
        const boxW = PRICE_AXIS_W - 18;
        const boxH = 16;
        ctx.fillStyle = palette.axisLabelBg;
        ctx.strokeStyle = palette.axisLabelStroke;
        roundRect(ctx, tx, ty, boxW, boxH, 6);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = palette.axisLabelText;
        ctx.textAlign = 'left';
    ...
    for (let i = firstVisible; i <= lastVisible; i++) {
        const c = candleData[i];
        const x = offsetX + (i * totalStep) + (candleW / 2);
        if (x + candleW / 2 < viewLeft || x - candleW / 2 > viewRight) continue;
        ...
        ctx.lineWidth = Math.max(1.2, 1.2 * scale);
        ctx.stroke();
        const bodyTop = Math.min(openY, closeY);
        const bodyH = Math.max(2, Math.abs(closeY - openY));
        const bodyX = x - candleW / 2;
        const bodyY = bodyTop;
        const r = Math.max(2, Math.min(5, candleW * 0.25));
        ctx.save();
        ctx.shadowBlur = 10;
        ctx.shadowColor = isUp ? 'rgba(0,230,118,0.12)' : 'rgba(255,23,68,0.12)';
        ctx.fillStyle = bodyFill;
        roundRect(ctx, bodyX, bodyY, candleW, bodyH, r);
        ctx.fill();
        ctx.restore();
        ctx.strokeStyle = bodyStroke;
        ctx.lineWidth = Math.max(1.2, 1.2 * scale);
        roundRect(ctx, bodyX + 0.5, bodyY + 0.5, candleW - 1, bodyH - 1, r);
        ctx.stroke();
        ctx.font = `800 ${Math.max(9, Math.min(12, candleW * 0.62))}px "Courier New"`;
        ctx.fillStyle = palette.candleNumber;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'bottom';
        ctx.fillText(String(c.number ?? ''), x, lowY - 4);

        const volVal = (c.high - c.low) + Math.abs(c.close - c.open);
        const volBarH = (volVal / maxVol) * (volBottom - volTop);
        ...
        if (i === hoveredIndex && showCrosshair) {
            ctx.strokeStyle = isLightTheme ? 'rgba(15,23,42,0.65)' : 'rgba(255,255,255,0.85)';
            ctx.lineWidth = 1.2;
            roundRect(ctx, bodyX - 2, bodyY - 2, candleW + 4, bodyH + 4, r + 2);
            ctx.stroke();
        }
    }
    ...
    if (last) {
        ...
        ctx.strokeStyle = palette.lastLine;
        ctx.setLineDash([6, 6]);
        ctx.beginPath();
        ctx.moveTo(0, lastY + 0.5);
        ctx.lineTo(plotW, lastY + 0.5);
        ctx.stroke();
        ctx.setLineDash([]);
        const txt = last.close.toFixed(3);
        const bx = plotW + 10;
        const by = clamp(lastY - 9, 6, chartHeight - 22);
        const bw = PRICE_AXIS_W - 18;
        const bh = 16;
        ctx.fillStyle = palette.lastBoxBg;
        ctx.strokeStyle = palette.lastBoxStroke;
        roundRect(ctx, bx, by, bw, bh, 7);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = palette.lastBoxText;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
    ...
    if (hoveredIndex >= 0 && showCrosshair) {
        const cx = clamp(lastMouseX, 0, plotW);
        const cy = clamp(lastMouseY, plotTop, plotBottom);
        ctx.strokeStyle = palette.crosshair;
        ctx.lineWidth = 1;
        ctx.setLineDash([3, 6]);
        ctx.beginPath();
        ctx.moveTo(cx + 0.5, plotTop);
        ctx.lineTo(cx + 0.5, volBottom);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(0, cy + 0.5);
        ctx.lineTo(plotW, cy + 0.5);
        ctx.stroke();
        ctx.setLineDash([]);
        const p = yToPrice(cy);
        const pTxt = p.toFixed(3);
        const bx = plotW + 10;
        const by = clamp(cy - 9, 6, chartHeight - 22);
        const bw = PRICE_AXIS_W - 18;
        const bh = 16;
        ctx.fillStyle = palette.priceBoxBg;
        ctx.strokeStyle = palette.priceBoxStroke;
        roundRect(ctx, bx, by, bw, bh, 7);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = palette.priceBoxText;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(pTxt, bx + 7, by + bh / 2);
        const c = candleData[hoveredIndex];
        if (c) {
            const tTxt = formatPeriodLabel(c.period);
            const tw = ctx.measureText(tTxt).width;
            const tbw = tw + 16;
            const tbh = 16;
            const tbx = clamp(cx - tbw / 2, 6, plotW - tbw - 6);
            const tby = volBottom - 10;
            ctx.fillStyle = palette.timeBoxBg;
            ctx.strokeStyle = palette.timeBoxStroke;
            roundRect(ctx, tbx, tby, tbw, tbh, 7);
            ctx.fill();
            ctx.stroke();
            ctx.fillStyle = palette.timeBoxText;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
    ...
    const lineColor = isLightTheme ? 'rgba(30, 41, 59, 0.9)' : 'rgba(255, 215, 0, 0.9)';
    const glowColor = isLightTheme ? 'rgba(59, 130, 246, 0.25)' : 'rgba(255, 215, 0, 0.25)';
    const drawOneLine = (ln, isPreview = false) => {
        if (!ln) return;
        ctx.save();
        ctx.lineWidth = isPreview ? 2.2 : 2.6;
        ctx.strokeStyle = lineColor;
        ctx.shadowBlur = 10;
        ctx.shadowColor = glowColor;
        ctx.setLineDash(isPreview ? [6, 6] : []);
        ctx.beginPath();
    ...
    userLines.forEach(l => drawOneLine(l, false));
    drawOneLine(previewLine, true);
}
...
function scrollToNewest() {
    if (!candleData.length) { offsetX = 40;
        drawChart(); return; }
    const PRICE_AXIS_W = 74;
    const plotW = chartWidth - PRICE_AXIS_W;
    const candleW = getCandleWidth();
...
function showTooltip(x, y, candle) {
    ...
    let rS = document.getElementById('tooltipResult');
    rS.innerText = candle.result;
...
function detectPatternsForIndex(idx) {
    ...
    const c1 = candleData[idx - 1];
    const c2 = candleData[idx - 2];
    let patterns = [];
...
function updatePatternsBar() {
    ...
    if (patterns.length === 0) {
        container.innerHTML = '<span class="pattern-badge pattern-neutral">No Pattern</span>';
    } else {
        let html = '';
        patterns.forEach(p => {
...
function saveLoginCredentials(username, password, platform) {
    try {
        localStorage.setItem(STORAGE_LOGIN, JSON.stringify({ username, password, platform }));
    } catch (e) { console.warn('Unable to save login credentials:', e); }
}

function restoreLoginCredentials() {
    try {
        const stored = localStorage.getItem(STORAGE_LOGIN);
        if (!stored) return;
    ...
    } catch (e) { console.warn('Unable to restore login credentials:', e); }
}
...
async function performLogin(username, password) {
    ...
    try {
        ...
        saveLoginCredentials(username, password, selectedPlatform);
...
function updateTimer() {
    ...
    const playBetCloseBeep = () => {
        if (!AudioContextClass) return;
        try {
            if (!beepAudioContext) beepAudioContext = new AudioContextClass();
            if (beepAudioContext.state === 'suspended') beepAudioContext.resume().catch(() => {});
            ...
            console.warn('Bet close beep blocked:', err);
        }
    };
    if (secs <= 11 && mins === 0) {
        playBetCloseBeep();
...
function showResultPopup(period, number, result, betType, totalBetAmount, balanceBefore, balanceAfter) {
    ...
    if ([1, 3, 7, 9].includes(number)) {
        colorName = "GREEN";
        colorStyle = "background: #00e676; color: #fff;";
    } else if ([2, 4, 6, 8].includes(number)) {
        colorName = "RED";
        colorStyle = "background: #ff1744; color: #fff;";
    } else if (number === 5) {
        colorName = "VIOLET/GREEN";
        colorStyle = "background: linear-gradient(90deg, #d500f9 50%, #00e676 50%); color: #fff;";
    } else if (number === 0) {
        colorName = "RED/VIOLET";
        colorStyle = "background: linear-gradient(90deg, #ff1744 50%, #d500f9 50%); color: #fff;";
    }
...
function startLineAt(point) {
    const b = getPlotBounds();
    const x = clamp(point.x, 0, b.plotW);
    const y = clamp(point.y, b.plotTop, b.plotBottom);
    isDrawingLine = true;
    lineStart = { x, y };
    previewLine = { x1: x, y1: y, x2: x, y2: y };
}

function updateLineTo(point) {
    if (!isDrawingLine || !lineStart) return;
    const b = getPlotBounds();
    const x2 = clamp(point.x, 0, b.plotW);
    const y2 = clamp(point.y, b.plotTop, b.plotBottom);
    previewLine = { x1: lineStart.x, y1: lineStart.y, x2, y2 };
}

function finishLine() {
    if (!isDrawingLine || !previewLine) return;
    userLines.push(previewLine);
    previewLine = null;
    isDrawingLine = false;
    lineStart = null;
    drawChart();
...
function setupInteractions() {
    canvas.addEventListener('mousemove', (e) => {
        ...
        const PRICE_AXIS_W = 74;
        const plotW = chartWidth - PRICE_AXIS_W;
        lastMouseX = clamp(mouseX, 0, plotW);
        lastMouseY = clamp(mouseY, 0, chartHeight);
        if (mouseX > plotW) { hoveredIndex = -1;
    ...
    window.addEventListener('mousemove', (e) => {
        if (drawMode && isDrawingLine) {
            updateLineTo(getPointerXY(e));
            drawChart();
            return;
        }
        if (isDragging) {
    ...
    canvas.addEventListener('wheel', (e) => {
        ...
        const PRICE_AXIS_W = 74;
        const plotW = chartWidth - PRICE_AXIS_W;
        mouseX = clamp(mouseX, 0, plotW);
        const oldScale = scale;
        const oldCandleW = getCandleWidth(oldScale);
        const oldGap = Math.max(1, oldCandleW * 0.12);
        const oldStep = oldCandleW + oldGap;
        const idxFloat = ((mouseX - offsetX) - oldCandleW / 2) / oldStep;
        const zoomFactor = e.deltaY > 0 ? 0.92 : 1.08;
        scale = clamp(scale * zoomFactor, 0.55, 2.4);
        const newCandleW = getCandleWidth(scale);
        const newGap = Math.max(1, newCandleW * 0.12);
        const newStep = newCandleW + newGap;
        offsetX = mouseX - (idxFloat * newStep + newCandleW / 2);
        drawChart();
    }, { passive: false });
...
window.addEventListener('load', () => {
    ...
    // Draw tools
    document.getElementById('drawLineBtn').onclick = () => setDrawMode(!drawMode);
    document.getElementById('clearLinesBtn').onclick = clearUserLines;
    ...
    if (phoneEl) {
        ...
        phoneEl.addEventListener('focus', () => {
            ensurePrefix();
            requestAnimationFrame(() => phoneEl.setSelectionRange(phoneEl.value.length, phoneEl.value.length));
        });
